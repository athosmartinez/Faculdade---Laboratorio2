...
namespace _08PilhaDinamica {
  // Pilha com Alocação dinâmica em C#: Repita o exercício de pilha, porém utilizando estruturas nativas (collections).
  class Program {
    static void Main(string[] args) {
      Stack<Jogador> pilha = new Stack<Jogador>(20);
    ...
    public static void Print(Stack<Jogador> pilha) {
      Stack<Jogador> stack = new Stack<Jogador>(pilha.ToArray());
      
      foreach (Jogador item in stack) {
        item.Imprimir();
      }
    }

    public static void InsereElementosIniciais(Stack<Jogador> pilha) {
      string word = "";
      do {
