...

{
    class Program
    {
        ...
        public static void quickSort(JogadorPrin[] lista, int esq, int dir)
        {
            int i = esq, j = dir;
            JogadorPrin pivo = lista[(esq + dir) / 2];

            while (i <= j)
            {
                while (lista[i].Nome.CompareTo(pivo.Nome) < 0)
                    i++;

                while (lista[j].Nome.CompareTo(pivo.Nome) > 0)
                    j--;
